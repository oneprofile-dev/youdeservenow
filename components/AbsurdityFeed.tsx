...
export default function AbsurdityFeed({ limit = 20, showFilters = true }: AbsurdityFeedProps) {
  const [sort, setSort] = useState<"legit" | "gaslight" | "controversial">("legit");
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {[
                { id: "legit", label: "✅ Most Legit", icon: "👍" },
                { id: "gaslight", label: "🔥 Most Challenged", icon: "🔥" },
                { id: "controversial", label: "⚔️ Most Debated", icon: "⚡" },
              ].map((option) => (
                <button
                  key={option.id}
      ...
      {results.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {results.map((result, idx) => (
            <AbsurdityCard key={result.id} result={result} rank={idx + 1} />
          ))}
        </div>
      )}
    </div>
